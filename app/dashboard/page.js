...
export default function DashboardPage() {
  ...
  async function loadUser() {
    try {
      ...
      if (!tabSessionId) { router.push("/login"); return }

      var userResult = await supabase.from("users").select("*, kelas(nama_kelas)").eq("id", session.user.id).single()
      ...
      if (userData.session_id && userData.session_id !== tabSessionId) {
        await forceLogout("Akun ini dibuka di tab lain.")
      ...
      setLoading(false)
      await sendHeartbeat()
    } catch (err) { console.log("loadUser:", err); setLoading(false) }
  }
  ...
  async function sendHeartbeat() {
    try {
      ...
      await supabase.from("users").update({ is_online: true, last_seen: new Date().toISOString() }).eq("id", userId).eq("session_id", tabSessionId)
    } catch (err) { console.log("heartbeat:", err) }
  }
  ...
  async function checkSession() {
    try {
      ...
      var tabSessionId = sessionStorage.getItem("app_session_id")
      if (!userId || !tabSessionId) return
      var result = await supabase.from("users").select("session_id").eq("id", userId).single()
      if (result.data && result.data.session_id !== tabSessionId) {
        await forceLogout("Akun login di tab lain.")
      }
    } catch (err) { console.log("checkSession:", err) }
  }
  ...
  async function markOffline() {
    try {
      ...
      var tabSessionId = sessionStorage.getItem("app_session_id")
      if (!userId) return
      await supabase.from("users").update({ is_online: false, last_seen: new Date().toISOString(), session_id: null }).eq("id", userId).eq("session_id", tabSessionId)
    } catch (err) { console.log("markOffline:", err) }
  }

  async function forceLogout(msg) {
    setKickMessage(msg); setKicked(true)
    sessionStorage.removeItem("app_session_id")
    sessionStorage.removeItem("app_user_id")
  ...
  if (kicked) {
    return (
      <div style={st.loadingContainer}>
        ...
        <p style={{ color: "#666", textAlign: "center", maxWidth: "400px" }}>{kickMessage}</p>
        <button onClick={function () { router.push("/login") }} style={st.loginAgainBtn}>🔑 Login Ulang</button>
      </div>
    )
  }

  if (loading) {
    return (<div style={st.loadingContainer}><div style={st.spinner}></div><p style={{ marginTop: "20px", color: "#666" }}>Loading...</p></div>)
  }

  if (!user) {
    return (<div style={st.loadingContainer}><p>User tidak ditemukan</p></div>)
  }
    ...
    <div style={st.container}>
        ...
        <div style={st.headerLeft}>
          ...
          <div>
            ...
            <div style={st.badgeRow}>
              <span style={{ ...st.badge, background: user.role === "guru" ? "#dbeafe" : "#dcfce7", color: user.role === "guru" ? "#1e40af" : "#166534" }}>
                {user.role === "guru" ? "👨‍🏫 Guru" : "🎓 Siswa"}
              </span>
              {user.kelas && <span style={st.kelasBadge}>🏫 {user.kelas.nama_kelas}</span>}
            </div>
          </div>
        </div>
      ...
      <div style={st.welcomeCard}>
        ...
        <p style={{ margin: "8px 0 0 0", opacity: 0.9 }}>
          {user.role === "guru" ? "Kelola pembelajaran dengan mudah dan efisien." : "Yuk, tingkatkan prestasi belajarmu!"}
        </p>
      </div>
      ...
      <div style={st.footer}><p style={{ margin: 0, color: "#999", fontSize: "14px" }}>© 2025 {namaApp}</p></div>
    </div>
  )
}
...
function MenuCard(props) {
  ...
  return (
    <div onClick={function () { props.router.push(props.link) }}
      onMouseEnter={function () { setHover(true) }} onMouseLeave={function () { setHover(false) }}
      style={{ ...st.card, transform: hover ? "translateY(-4px)" : "translateY(0)", boxShadow: hover ? "0 10px 25px rgba(0,0,0,0.1)" : "0 2px 8px rgba(0,0,0,0.04)" }}>
      <div style={{ ...st.cardIcon, background: props.color + "15" }}><span style={{ fontSize: "28px" }}>{props.emoji}</span></div>
...
var st = {
  ...
  loadingContainer: { minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", background: "#f5f7fa" },
  ...
  loginAgainBtn: { marginTop: "20px", padding: "12px 28px", background: "linear-gradient(135deg, #667eea, #764ba2)", color: "white", border: "none", borderRadius: "10px", cursor: "pointer", fontWeight: "700", fontSize: "15px" },
  header: { background: "white", padding: "24px 28px", borderRadius: "16px", marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", flexWrap: "wrap", gap: "16px" },
  headerLeft: { display: "flex", alignItems: "center", gap: "16px" },
  logoWrap: { cursor: "pointer", flexShrink: 0 },
  logoImg: { width: "50px", height: "50px", borderRadius: "12px", objectFit: "cover" },
  logoPlaceholder: { width: "50px", height: "50px", borderRadius: "12px", background: "#f3f4f6", display: "flex", justifyContent: "center", alignItems: "center", fontSize: "28px" },
  avatarWrap: { position: "relative", flexShrink: 0 },
  avatarContainer: { cursor: "pointer", position: "relative" },
  avatarImg: { width: "60px", height: "60px", borderRadius: "50%", objectFit: "cover", border: "3px solid #667eea" },
  avatar: { width: "60px", height: "60px", borderRadius: "50%", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", color: "white", display: "flex", justifyContent: "center", alignItems: "center", fontSize: "24px", fontWeight: "bold" },
  editBadge: { position: "absolute", bottom: "-2px", right: "-2px", width: "24px", height: "24px", borderRadius: "50%", background: "#667eea", display: "flex", justifyContent: "center", alignItems: "center", fontSize: "12px", border: "2px solid white" },
  ...
  kelasBadge: { display: "inline-block", padding: "4px 12px", borderRadius: "20px", fontSize: "12px", fontWeight: "600", background: "#fef3c7", color: "#92400e" },
  logoutBtn: { padding: "10px 20px", background: "white", color: "#dc2626", border: "2px solid #fecaca", borderRadius: "10px", cursor: "pointer", fontWeight: "600", fontSize: "14px" },
  uploadCard: { background: "white", padding: "20px", borderRadius: "12px", marginBottom: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" },
  welcomeCard: { background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", color: "white", padding: "28px", borderRadius: "16px", marginBottom: "24px", boxShadow: "0 4px 12px rgba(102, 126, 234, 0.3)" },
  menuTitle: { marginBottom: "16px" },
  grid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "16px", marginBottom: "40px" },
  card: { background: "white", padding: "24px", borderRadius: "16px", cursor: "pointer", transition: "all 0.3s ease" },
  cardIcon: { width: "56px", height: "56px", borderRadius: "14px", display: "flex", justifyContent: "center", alignItems: "center", marginBottom: "16px" },
